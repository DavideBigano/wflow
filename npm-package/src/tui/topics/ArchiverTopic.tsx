...
import { findRunningId, listStashedRuns, reviveRun, stashRunningRun } from "../../lib/runArchive.js";
import { describeError } from "../../lib/wflowError.js";
import { ConfirmPrompt } from "./ConfirmPrompt.js";

type ArchiverRow = { kind: "active"; runId: string } | { kind: "stashed"; runId: string } | { kind: "refresh" };
...
export function ArchiverTopic({ workspaceRoot, onModalStateChange }: ArchiverTopicProps) {
  ...
  const rows: ArchiverRow[] = [
    ...(activeRunId ? ([{ kind: "active", runId: activeRunId }] as ArchiverRow[]) : []),
    ...stashedRunIds.map((runId): ArchiverRow => ({ kind: "stashed", runId })),
    { kind: "refresh" },
  ];

  useEffect(() => {
    setSelected((current) => Math.max(0, Math.min(current, rows.length - 1)));
  ...
  const executeConfirm = useCallback(async () => {
    ...
    try {
      if (row.kind === "active") {
        ...
        const result = await reviveRun(workspaceRoot, row.runId, { autoStash: true });
        setStatusMessage(
          result.stashedRunId
  ...
  useInput((input, key) => {
    ...
    else if (key.return && rows[selected]?.kind === "refresh") void refresh();
  ...
  return (
    <Box flexDirection="column">
      <Box borderStyle="single" borderColor={isActiveSelected ? "cyan" : undefined} paddingX={1} marginBottom={1}>
        <Text>
          {isActiveSelected ? "› " : "  "}
          Running: {activeRunId ? <Text color="green">{activeRunId}</Text> : <Text dimColor>none</Text>}
        </Text>
      </Box>

      <Box flexDirection="column" marginBottom={1}>
        {stashedRunIds.length === 0 && <Text dimColor>no archived runs</Text>}

        {scrollOffset > 0 && <Text dimColor>↑ {scrollOffset} more</Text>}

        {visible.map((runId, i) => {
          const realIndex = scrollOffset + i;
          const isSelected = !isActiveSelected && !isRefreshSelected && realIndex === stashedSelectedIndex;
          return (
            <Text key={runId} color={isSelected ? "cyan" : undefined}>
              {isSelected ? "› " : "  "}
              {runId}
            </Text>
          );
        ...
        {scrollOffset + VIEWPORT < stashedRunIds.length && (
          <Text dimColor>↓ {stashedRunIds.length - scrollOffset - VIEWPORT} more</Text>
        )}
      ...
      {confirmOpen && <ConfirmPrompt prompt={confirmMessage} isYesActive={confirmYes} />}

      {!confirmOpen && statusMessage && <Text dimColor>{statusMessage}</Text>}
...
function computeScrollOffset(selectedIndex: number, total: number, viewport: number): number {
