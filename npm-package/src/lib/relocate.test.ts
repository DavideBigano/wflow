...
test("relocateTree moves every file from source to the equivalent destination path", async () => {
  ...
  await relocateTree("/ws/stages/01-intake/outputs", "/ws/archive/run-1/01-intake", fs);
...
test("relocateTree preserves nested subdirectory structure at the destination", async () => {
  ...
  await expect(fs.readTextFile("/ws/archive/run-1/01-intake/notes/note.md")).resolves.toBe("hello");
});
...
test("relocateTree returns moved files as paths relative to source", async () => {
  ...
  const moved = await relocateTree("/ws/stages/01-intake/outputs", "/ws/archive/run-1/01-intake", fs);
...
test("relocateTree removes the emptied source root when removeRootWhenEmpty is set", async () => {
  ...
  await relocateTree("/ws/archive/run-1/01-intake", "/ws/stages/01-intake/outputs", fs, {
    removeRootWhenEmpty: true,
  });

  await expect(fs.pathExists("/ws/archive/run-1/01-intake")).resolves.toBe(false);
});
...
test("relocateTree leaves the source root behind when removeRootWhenEmpty is not set", async () => {
  ...
  await expect(fs.pathExists("/ws/stages/01-intake/outputs/idea.json")).resolves.toBe(true);
});

test("relocateTree on a missing source returns an empty list", async () => {
  const fs = createInMemoryFilesystemGateway();

  const moved = await relocateTree("/ws/does-not-exist", "/ws/destination", fs);
