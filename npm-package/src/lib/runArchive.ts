...
import { nodeFilesystemGateway, type FilesystemGateway } from "./filesystemGateway.js";
import { readRunId } from "./runMetadata.js";
import { relocateTree } from "./relocate.js";
import { discoverOutputUnits, resolveArchiveDir, resolveRunFile } from "./workspacePaths.js";
...
export async function findRunningId(workspaceRoot: string, fs: FilesystemGateway = nodeFilesystemGateway): Promise<string | null> {
...
export async function listStashedRuns(workspaceRoot: string, fs: FilesystemGateway = nodeFilesystemGateway): Promise<string[]> {
  ...
  const withRunFile = await Promise.all(
    dirs.map(async (runId) => {
      const hasRunFile = await fs.pathExists(path.join(archiveDir, runId, "run.json"));
      return hasRunFile ? runId : null;
    })
  );

  return withRunFile.filter((runId): runId is string => runId !== null).sort((a, b) => a.localeCompare(b));
}

/** Moves the live run's output units and run.json into archive/<run-id>/, emptying the live layers. */
export async function stashRunningRun(workspaceRoot: string, fs: FilesystemGateway = nodeFilesystemGateway): Promise<StashResult> {
  ...
  for (const unit of units) {
    const moved = await relocateTree(unit.sourceDir, path.join(archiveRunDir, unit.name), fs, {
      removeRootWhenEmpty: false,
    });
    if (moved.length > 0) movedUnits.push(unit.name);
  }

  await fs.move(resolveRunFile(workspaceRoot), path.join(archiveRunDir, "run.json"));
...
export async function reviveRun(
  ...
  const archiveRunDir = path.join(resolveArchiveDir(workspaceRoot), targetRunId);
  ...
  if (!(await fs.pathExists(archiveRunDir))) {
    ...
    throw new WflowError(
      ...
      known.length > 0 ? `known archived run ids: ${known.join(", ")}` : "the archive/ folder is empty; nothing to restore."
    );
  }
  ...
  const units = await discoverOutputUnits(workspaceRoot, fs);
  for (const unit of units) {
    await relocateTree(path.join(archiveRunDir, unit.name), unit.sourceDir, fs, {
      removeRootWhenEmpty: true,
    });
  }

  return { revivedRunId: targetRunId, stashedRunId: stashedRunIdFromAutoStash };
}
