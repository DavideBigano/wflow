...
import { InputEventProvider, useInputListener } from './InputEventProvider.js';

const KEY_X = 'x';
const UP = '\u001B[A';

/** ink attaches its raw-mode input listener in an effect after mount, and registration effects run asynchronously; flush a macrotask to let both settle. */
function flush() {
...
describe('InputEventProvider', () => {
    ...
    test('unmounting a listener removes it from the stack', async () => {
        ...
        rerender(<InputEventProvider>{null}</InputEventProvider>);
        await flush();
    ...
    test('useInputListener throws when used outside an InputEventProvider', () => {
        function Bad() {
            useInputListener(() => {});
            return null;
        }
