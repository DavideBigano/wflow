 ...
 */
export function createInMemoryFilesystemGateway(initialFiles: Record<string, string> = {}): FilesystemGateway {
  const files = new Map<string, string>(Object.entries(initialFiles).map(([key, value]) => [normalize(key), value]));
  ...
  return {
    ...
    async move(from, to) {
      ...
      if (content === undefined) throw new Error(`fake fs: cannot move missing file ${from}`);
    ...
    async readTextFile(target) {
      ...
      if (content === undefined) throw new Error(`fake fs: no such file ${target}`);
      return content;
    },
  };
}
