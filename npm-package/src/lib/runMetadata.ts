...
export async function readRunId(runFilePath: string, fs: FilesystemGateway = nodeFilesystemGateway): Promise<string> {
  ...
  try {
    ...
    throw new WflowError(`run.json at ${runFilePath} is not valid JSON`, "fix or regenerate the file before continuing.");
  }
