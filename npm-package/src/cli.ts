...
function generateCompletionScript(): string {
  return execFileSync(process.execPath, [process.argv[1], "completion"], { encoding: "utf8" });
}
...
async function installCompletionScript(): Promise<void> {
  ...
  if (existing.includes(COMPLETION_MARKER)) {
    console.log(`completions already installed in ${rcFile} — nothing to do.`);
    return;
  }
        ...
        topic
          .command(
            ...
            async () => {
              ...
              const unitsLabel = result.movedUnits.length > 0 ? result.movedUnits.join(", ") : "(nothing to move)";
              console.log(`archived "${result.runId}" -> ${unitsLabel}`);
            }
          )
          .command(
            "restore <run-id>",
            ...
            (subcommand) =>
              subcommand
                .positional("run-id", { type: "string", describe: "Run id to restore", demandOption: true })
                .option("auto-archive", {
                  alias: "a",
                  type: "boolean",
                  default: false,
                  describe: "Archive the currently running run automatically before restoring",
                }),
            async (argv) => {
              const result = await reviveRun(process.cwd(), argv.runId, { autoStash: argv.autoArchive });
              if (result.stashedRunId) console.log(`auto-archived running run "${result.stashedRunId}"`);
